...
select avg(orderquantity) as ave_orderquantity from sales2016;
...
SELECT 
    OrderDate,
    CASE
        WHEN CAST(OrderDate AS CHAR) LIKE '%-%-%' AND STR_TO_DATE(CAST(OrderDate AS CHAR), '%m-%d-%Y') IS NOT NULL 
            THEN STR_TO_DATE(CAST(OrderDate AS CHAR), '%m-%d-%Y')
        WHEN CAST(OrderDate AS CHAR) LIKE '%/%/%' AND STR_TO_DATE(CAST(OrderDate AS CHAR), '%m/%d/%Y') IS NOT NULL 
            THEN STR_TO_DATE(CAST(OrderDate AS CHAR), '%m/%d/%Y')
        ELSE NULL
    END AS formatted_date
...
CREATE INDEX idx_sales2016_productkey ON sales2016 (ProductKey);
